...
export function PropertyHighlightsSection() {
  ...
  return (
    <section className="border-b border-zinc-200 bg-[#fff8ed] py-16 sm:py-20">
        ...
        <div className="mt-10 grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
          {propertyHighlights.map((item, index) => (
            <motion.article
              ...
              className="group overflow-hidden rounded-xl border border-zinc-200 bg-[#f5efe4] shadow-sm transition hover:border-sky-300 hover:shadow-soft-panel"
            >
              <div className="relative aspect-[1.18] overflow-hidden">
                ...
                <div className="absolute left-4 top-4 grid h-11 w-11 place-items-center rounded-lg bg-[#fff8ed] text-zinc-950 shadow-lg ring-1 ring-sky-100">
                  <item.icon className="h-5 w-5" />
                </div>
                <span className="absolute bottom-4 left-4 rounded-lg border border-[#fff8ed]/15 bg-[#fff8ed]/92 px-3 py-2 text-xs font-semibold text-sky-800 shadow-lg backdrop-blur-md">
                  {item.metric}
                </span>
              </div>
